...
void processMessage( Frame& message )
{
  ...
  { 
    ...
    dataFile.println(msgCount++, DEC);
    dataFile.println(",");
    dataFile.println(getTimeDifference(),DEC);
    dataFile.println(",");
    dataFile.println(message.id,HEX);
    dataFile.println(",");
    dataFile.println(message.dlc,DEC);  
    dataFile.println(",");       
...
void preparaSD(void) {
 ...
 //If the file exists change the name until a new one is created
  ...
  if (dataFile) {
    Serial.println("StoFile.txt:");
    // read from the file until there's nothing else in it:
    while (dataFile.available()) {
      Serial.write(dataFile.read());
      dataFile.println(header);
      dataFile.flush();
      Serial.print("STO CAZZO");
    }
    // close the file:
  } 
  else {
